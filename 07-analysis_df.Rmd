...
```{r datacolor, echo=F}
starburst.ct <-  data.frame(table(subset(ct.no_scent, type == "starburst")$flavor))
colnames(starburst.ct) <- c("Flavor", "Starburst")

mm.ct <-  data.frame(table(subset(ct.no_scent, type == "m&m")$flavor))
colnames(mm.ct) <- c("Flavor", "M&M")

pean.ct <-  data.frame(table(subset(ct.no_scent, type == "peanut m&m")$flavor))
