...
![unchanged image](MATLAB_open.png)
...
#### Question 3
  ...
  - Create a new vector, $y$, with each entry equal to the cube of the corresponding entry in $z$. 
  - What is the 22nd entry of $y$?
...
```{block, type='rmdquestion'}
#### Question 4
  - Using the same matrices, $A$ and $B$ that you used in Question 3, create two new matrices, $C$ and $D$.  
    $C$ should be the matrix obtained by multiplying $A$ and $B$ using `matrix multiplication`, while $D$ should be the matrix obtained by `element-wise multiplication`.
